...
class ZeroShotLearningOld:
    ...
    def start(self):
        ...
        self.seen_embedding = model.encode(self.seen["features"].values.tolist())
        self.unseen_embedding = model.encode(self.unseen["features"].values.tolist())

        self.embedding = {
            "seen": self.seen_embedding,
            "unseen": self.unseen_embedding
        }

        predictions = []

        for embedding in self.unseen_embedding:
            score = cosine_similarity([embedding], self.seen_embedding)[0]
        ...
        if self.config.get_nested('evaluate', 'silhouette'):
            log(f"Computing Silhouette Average...",
                verbose=self.verbose)
            silhouette_avg = silhouette_score(self.unseen_embedding, predictions)
