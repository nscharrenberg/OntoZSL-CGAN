...
class OntoZSLDataSampler(object):
    ...
    def __init__(self, data, rds, output_info, log_frequency, embedding=None):
            ...
            if is_discrete_column(column_info_primary):
                ...
                for index_secondary in range(index_primary + 1, len(output_info)):
                    ...
                    if is_discrete_column(column_info_secondary):
                        ...
                        rid_by_cat_pair = []
                        combine_pair_data = np.append(data[:, st_primary:ed_primary],
                                                      data[:, st_secondary:ed_secondary], axis=1)
        ...
        self._discrete_pair_cond_st = np.zeros((int(((n_discrete_columns) * (n_discrete_columns - 1)) / 2),
                                                int((max_category + 1) * (second_max_category + 1))), dtype='int32')
        self._discrete_pair_n_category = np.zeros(int(((n_discrete_columns) * (n_discrete_columns - 1)) / 2),
                                                  dtype='int32')
        self._discrete_column_pair_prob = np.zeros((int(((n_discrete_columns) * (n_discrete_columns - 1)) / 2),
                                                    int((max_category + 1) * (second_max_category + 1))))
            ...
            if is_discrete_column(column_info_primary):
                ...
                for index_secondary in range(index_primary + 1, len(output_info)):
                    ...
                    if is_discrete_column(column_info_secondary):
                        ...
                        combine_pair_data = np.append(data[:, 0:self._rd_col_dim], data[:, st_primary:ed_primary],
                                                      data[:, st_secondary:ed_secondary], axis=1)
                        combine_pair_data_decimal = reduce(lambda a, b: 2 * a + b, combine_pair_data.transpose())
                        unique_pair, counts_pair = np.unique(combine_pair_data_decimal,
                                                             return_counts=True)  # counts_pair --> category_freq
                        ...
                        self._discrete_pair_cond_st[current_id_pair,
                        0:len(unique_pair)] = unique_pair  ## current_cond_st_pair
    ...
    def sample_condvec_pair(self, batch):
        ...
        for iter_gen in range(0, batch):
            paired_discrete_column_id.append(
                np.concatenate(([0], np.random.choice(np.arange(self._n_discrete_columns), 2, replace=False))))
        ...
        pair_id_in_col = self._random_choice_prob_pairs(
            converted_paired_discrete_column_id)  # category_id_in_col: (0, max_num_cateogies), --> [0,2,1,0,2,1...]
        ...
        for itr_decimal in range(0, len(pair_id_decimal)):
            ...
            pair_secondary_position = np.argmax(pair_id_binary[first_cat:first_cat + second_cat])
            pair_tertiary_position = np.argmax(pair_id_binary[first_cat + second_cat:])
    ...
    def sample_original_condvec(self, batch):
        ...
        for i in range(batch):
            ...
            for idx in col_idx:
                mask[i, idx] = 1
                matrix_st = self._discrete_column_matrix_st[idx]
                matrix_ed = matrix_st + self._discrete_column_n_category[idx]
                pick = np.argmax(self._data[row_idx, matrix_st:matrix_ed])
                cond[i, pick + self._discrete_column_cond_st[idx]] = 1
    ...
    def get_embeds_from_cat_ids(self, cat_ids, batch_size):
        """
        Retrieves embeddings for categorical IDs.

        Parameters:
            cat_ids (numpy.ndarray): Categorical IDs for which embeddings are required.
            batch_size (int): The number of embeddings to retrieve.

        Returns:
            numpy.ndarray: The embeddings corresponding to the provided categorical IDs.
        """
    ...
    def get_rds(self, cat_ids, batch_size):
        """
        Retrieves Rare Diseases (RD) for the provided categorical IDs.

        Parameters:
            cat_ids (numpy.ndarray): Categorical IDs for which RD values are required.
            batch_size (int): The number of RD values to retrieve.

        Returns:
            list: The RD values corresponding to the provided categorical IDs.
        """
        rds = []
    ...
    def get_rd_embeds(self, rds):
        """
        Retrieves embeddings for the provided Rare Diseases (RD).

        Parameters:
            rds (list): RD values for which embeddings are required.

        Returns:
            numpy.ndarray: The embeddings corresponding to the provided RD values.
        """
        embed_size = self._embedding.embed_size
