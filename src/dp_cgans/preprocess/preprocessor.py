...
class Preprocessor:
    ...
    def _xml_to_csv(self):
        ...
        log(text=f"Success! XML has been converted to CSV and saved to \"{new_file}\"", verbose=True)
    ...
    def _csv_to_txt(self):
        ...
        if not os.path.isfile(csv_file):
            raise Exception(
                f"The CSV file located at \"{csv_file}\" does not exist, and can therefore not be converted.")

        if os.path.isfile(new_file):
            raise Exception(f"The TXT file located at \"{new_file}\" already exists, and can therefore not be saved.")
    ...
    def _create_training_and_test_dataset(self):
        ...
        if os.path.isfile(seen_file):
            raise Exception(
                f"The seen file located at \"{seen_file}\" already exists, and can therefore not be created.")

        if os.path.isfile(unseen_file):
            raise Exception(f"The unseen file located at \"{unseen_file}\" already exists, and can therefore not be created.")
        ...
        log(text=f"Success! The Seen and Unseen Datasets have been created.", verbose=True)
    ...
    def _generate(self):
        ...
        if not os.path.isfile(seen_file):
            raise Exception(
                f"The seen file located at \"{seen_file}\" does not exist, and can therefore not be processed.")

        if not os.path.isfile(unseen_file):
            raise Exception(f"The unseen file located at \"{unseen_file}\" does not exist, and can therefore not be processed.")
        ...
        if os.path.isfile(save_seen_path):
            raise Exception(
                f"The seen patient dataset located at \"{save_seen_path}\" already exists, and can therefore not be saved.")

        if os.path.isfile(save_unseen_path):
            raise Exception(
                f"The unseen patient dataset located at \"{save_unseen_path}\" already exists, and can therefore not be saved.")
        ...
        if not os.path.isfile(csv_file):
            raise Exception(f"The hpo file located at \"{csv_file}\" does not exist, and can therefore not be processed.")
        ...
        if gen_small_file:
            log(f"📖 Progress: Minify Data to ensure smaller file sizes...", verbose=self.verbose)
        ...
        log(f"📖 Info: There are {rd_count} unique Rare Diseases.", verbose=self.verbose)
        log(f"📖 Info: There are {total_hp_count} unique Phenotypes.", verbose=self.verbose)
                ...
                if len(temp_hp) > 0:
                    ...
                    if print_every > 0:
                        if patients_count % print_every == 0:
                            log(f"{patients_count} / {patients_per_rd * rd_count} patients generated")
        ...
        log(f"Total Rare Diseases: {rd_count}", verbose=self.verbose)
        log(f"{rd_count - len(unseen_unique)} seen Rare Diseases", verbose=self.verbose)
        log(f"{len(unseen_unique)} unseen Rare Diseases", verbose=self.verbose)
    ...
    def _generate_dictionaries(self):
        ...
        if os.path.isfile(dict_hpo_file):
            raise Exception(
                f"The HPO dictionary file located at \"{dict_hpo_file}\" already exists, and can therefore not be saved.")

        if os.path.isfile(dict_ordo_file):
            raise Exception(
                f"The ORDO dictionary file located at \"{dict_ordo_file}\" already exists, and can therefore not be saved.")
